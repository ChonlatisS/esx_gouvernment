...
ESX.RegisterServerCallback('esx_gouvernmentjob:getVehicleFromGouvernment', function (source, cb)
	MySQL.Async.fetchAll('SELECT *, date_format(time, "%Y-%m-%d") AS time FROM owned_vehicles WHERE gouvernment = @gouvernment', {
		['@gouvernment'] =1
	}, function (result)
		...
		for i=1, #result, 1 do
			...
			table.insert(vehicles, {
				...
				gouvernment		  = result[i].gouvernment,
				gouvernment_by		  = result[i].gouvernment_by,
				time		  = result[i].time
			})
		end
...
RegisterServerEvent('esx_gouvernmentjob:updateVehicleFromGouvernment')
AddEventHandler('esx_gouvernmentjob:updateVehicleFromGouvernment', function(plate)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.execute('UPDATE owned_vehicles SET `gouvernment` = @gouvernment, `gouvernment_by` = @gouvernment_by, `time` = NOW() WHERE plate = @plate', {
		['@gouvernment'] = 1,
		['@plate'] = plate,
		['@gouvernment_by'] = xPlayer.name
	}, function(rowsChanged)
		if rowsChanged == 0 then
			print("ERROR Update setVehicleFromGouvernment")
		end
	end)
end)


RegisterServerEvent('esx_gouvernmentjob:setVehicleFromGouvernment')
AddEventHandler('esx_gouvernmentjob:setVehicleFromGouvernment', function(plate)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored, `gouvernment` = @gouvernment, `gouvernment_by` = @gouvernment_by WHERE plate = @plate', {
		['@stored'] = false,
		['@gouvernment'] = 0,
		['@plate'] = plate,
		['@gouvernment_by'] = nil
...
AddEventHandler('esx_gouvernmentjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if amount == nil then
		TriggerClientEvent("pNotify:SendNotification", _source, {
			text = '<center><strong class="red-text">ปริมาณที่ไม่ถูกต้อง</strong><center>',
			type = "error",
			timeout = 3000,
			layout = "centerLeft",
			queue = "global"
	...
	if itemType == 'item_standard' then
		...
		if targetItem.count > 0 then
			...
			if targetItem.count < amount then
				...
				TriggerClientEvent("pNotify:SendNotification", _source, {
					text = '<center><strong class="red-text">จำนวนที่ไม่ถูกต้อง</strong><center>',
					type = "error",
					timeout = 3000,
					layout = "centerLeft",
					queue = "global"
				...
				if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
					TriggerClientEvent("pNotify:SendNotification", target, {
						text = '<center><strong class="red-text">ช่องเก็บของมีพื้นที่ไม่เพียงพอ</strong><center>',
						type = "error",
						timeout = 3000,
						layout = "centerLeft",
						queue = "global"
					...
					TriggerClientEvent("pNotify:SendNotification", _source, {
						text = '<center>คุณได้ริบ <strong class="amber-text">'..sourceItem.label..'</strong> จำนวน <strong class="amber-text">'..amount..'</strong> ของคุณ <strong class="blue-text">'..targetXPlayer.name..'</strong> เรียบร้อยแล้ว<center>',
						type = "success",
						timeout = 3000,
						layout = "centerLeft",
						queue = "global"
					...
					TriggerClientEvent("pNotify:SendNotification", target, {
						text = '<center>บางคน <strong class="amber-text">'..sourceItem.label..'</strong> จำนวน <strong class="amber-text">'..amount..'</strong> ออกจากกระเป๋าคุณ<center>',
						type = "success",
						timeout = 3000,
						layout = "centerLeft",
						queue = "global"
			...
			TriggerClientEvent("pNotify:SendNotification", _source, {
				text = '<center><strong class="red-text">ผู้เล่นไม่มีไอเทม '..targetItem.label..'</strong><center>',
				type = "error",
				timeout = 3000,
				layout = "centerLeft",
				queue = "global"
	...
	elseif itemType == 'item_money' then
		if amount <= 0 then
			...
			if targetMoney < amount then
				...
				TriggerClientEvent("pNotify:SendNotification", _source, {
					text = '<center>คุณได้ริบ <strong class="amber-text">เงินสด</strong> จำนวน <strong class="amber-text">'..amount..'</strong> ของคุณ <strong class="blue-text">'..targetXPlayer.name..'</strong> เรียบร้อยแล้ว<center>',
					type = "success",
					timeout = 3000,
					layout = "centerLeft",
					queue = "global"
				...
				TriggerClientEvent("pNotify:SendNotification", target, {
					text = '<center>บางคน <strong class="amber-text">เงินสด</strong> จำนวน <strong class="amber-text">'..amount..'</strong> ออกจากกระเป๋าคุณ<center>',
					type = "success",
					timeout = 3000,
					layout = "centerLeft",
					queue = "global"
	...
	elseif itemType == 'item_account' then
		if amount <= 0 then
			TriggerClientEvent("pNotify:SendNotification", _source, {
				text = '<center><strong class="red-text">จำนวนที่ไม่ถูกต้อง</strong>'..amount..'<center>',
				type = "error",
				timeout = 3000,
				layout = "centerLeft",
				queue = "global"
			...
			if targetMoney < amount then
				...
				TriggerClientEvent("pNotify:SendNotification", _source, {
					text = '<center>คุณได้ริบ <strong class="amber-text">'..itemName..'</strong> จำนวน <strong class="amber-text">'..amount..'</strong> ของคุณ <strong class="blue-text">'..targetXPlayer.name..'</strong> เรียบร้อยแล้ว<center>',
					type = "success",
					timeout = 3000,
					layout = "centerLeft",
					queue = "global"
				...
				TriggerClientEvent("pNotify:SendNotification", target, {
					text = '<center>บางคน <strong class="amber-text">'..itemName..'</strong> จำนวน <strong class="amber-text">'..amount..'</strong> ออกจากกระเป๋าคุณ<center>',
					type = "success",
					timeout = 3000,
					layout = "centerLeft",
					queue = "global"
	...
	elseif itemType == 'item_weapon' then
		...
		TriggerClientEvent("pNotify:SendNotification", _source, {
			text = '<center>คุณได้ริบ <strong class="amber-text">'..ESX.GetWeaponLabel(itemName)..'</strong> จำนวนกระสุน <strong class="amber-text">'..amount..'</strong> ของคุณ <strong class="blue-text">'..targetXPlayer.name..'</strong> เรียบร้อยแล้ว<center>',
			type = "success",
			timeout = 3000,
			layout = "centerLeft",
			queue = "global"
		...
		TriggerClientEvent("pNotify:SendNotification", target, {
			text = '<center>บางคน <strong class="amber-text">'..ESX.GetWeaponLabel(itemName)..'</strong> จำนวนกระสุน <strong class="amber-text">'..amount..'</strong> ออกจากกระเป๋าคุณ<center>',
			type = "success",
			timeout = 3000,
			layout = "centerLeft",
			queue = "global"
	...
	elseif itemType == "item_key" then -- MEETA GiveKey
		...
		TriggerClientEvent("pNotify:SendNotification", _source, {
			text = '<center>คุณได้ริบ <strong class="amber-text">กุญแจรถ</strong> ทะเบียน <strong class="yellow-text">'..itemName..'</strong><center>',
			type = "success",
			timeout = 3000,
			layout = "centerLeft",
			queue = "global"
		...
		TriggerClientEvent("pNotify:SendNotification", target, {
			text = '<center>บางคน <strong class="amber-text">กุญแจรถ</strong> ทะเบียน <strong class="yellow-text">'..itemName..'</strong> ออกจากกระเป๋าคุณ<center>',
			type = "success",
			timeout = 3000,
			layout = "centerLeft",
			queue = "global"
	...
	elseif itemType == "item_keyhouse" then -- MEETA GiveKeyHouse
		...
		TriggerClientEvent("pNotify:SendNotification", _source, {
			text = '<center>คุณได้ริบ <strong class="amber-text">กุญแจบ้าน</strong><center>',
			type = "success",
			timeout = 3000,
			layout = "centerLeft",
			queue = "global"
		...
		TriggerClientEvent("pNotify:SendNotification", target, {
			text = '<center>บางคน <strong class="amber-text">กุญแจบ้าน</strong> ถูกยึดโดย <strong class="blue-text">'.. sourceXPlayer.name ..'</strong> ออกจากกระเป๋าคุณ<center>',
			type = "success",
			timeout = 3000,
			layout = "centerLeft",
			queue = "global"
...
AddEventHandler('esx_gouvernmentjob:handcuff', function(target, playerheading, playerCoords, playerlocation)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'gouvernment' then
		--TriggerClientEvent('esx_gouvernmentjob:handcuff', target)
		TriggerClientEvent('esx_gouvernmentjob:getarrested', target, playerheading, playerCoords, playerlocation)
    	TriggerClientEvent('esx_gouvernmentjob:doarrested', source)
	else
		print(('esx_gouvernmentjob: %s attempted to handcuff a player (not cop)!'):format(xPlayer.identifier))
	end
end)

RegisterServerEvent('esx_gouvernmentjob:unhandcuff')
AddEventHandler('esx_gouvernmentjob:unhandcuff', function(target, playerheading, playerCoords, playerlocation)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'gouvernment' then
		--TriggerClientEvent('esx_gouvernmentjob:handcuff', target)
		TriggerClientEvent('esx_gouvernmentjob:getuncuffed', target, playerheading, playerCoords, playerlocation)
    	TriggerClientEvent('esx_gouvernmentjob:douncuffing', source)
	else
		print(('esx_gouvernmentjob: %s attempted to handcuff a player (not cop)!'):format(xPlayer.identifier))
	end
end)
