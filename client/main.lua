...
function OpenCloakroomMenu()
	...
	table.insert(elements, {label = '<strong class="Green-text">ชุดประชาชน</strong>', value = 'citizen_wear'})
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			if type == 'car' then
				shopCoords = Config.GouvernmentStations[station].Vehicles[partNum].InsideShop
			...
			elseif type == 'helicopter' then
				shopCoords = Config.GouvernmentStations[station].Helicopters[partNum].InsideShop
		...
		elseif data.current.action == 'police_vehicle' then
			...
			local garage_gouvernment = {}
			ESX.TriggerServerCallback('esx_gouvernmentjob:getVehicleFromGouvernment', function(jobVehicles)
				if #jobVehicles > 0 then
					for k,v in ipairs(jobVehicles) do
						local props = json.decode(v.vehicle)
						local vehicleName = GetLabelText(GetDisplayNameFromVehicleModel(props.model))
						...
						if v.gouvernment_by then
							label = label .. ('โดย: %s'):format(v.gouvernment_by).." ".. v.time
						else
							label = label .. ('โดย: %s'):format("Unknow").." ".. v.time
						end

						table.insert(garage_gouvernment, {
							label = label,
							value = k
							-- stored = v.stored,
							-- value = props
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_garage1', {
						...
						if foundSpawn then
							...
							ESX.Game.SpawnVehicle(props.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
								...
								TriggerServerEvent('esx_gouvernmentjob:setVehicleFromGouvernment', props.plate)
								TriggerEvent("pNotify:SendNotification", {
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_gouvernmentjob:storeNearbyVehicle', function(storeSuccess, foundNum)
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	local spawnPoints = Config.GouvernmentStations[station][part][partNum].SpawnPoints
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm',
		{
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_gouvernmentjob:buyJobVehicle', function (bought)
					if bought then
						TriggerEvent("pNotify:SendNotification", {
...
function OpenGouvernmentActionsMenu()
	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'gouvernment_actions',
	{
		title    = 'Gouvernment',
		align    = 'top-left',
		elements = {
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						...
						TriggerServerEvent('esx_gouvernmentjob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
						ESX.Streaming.RequestAnimDict("amb@code_human_gouvernment_investigate@idle_a", function()
							TaskPlayAnim(GetPlayerPed(-1), 'amb@code_human_gouvernment_investigate@idle_a', 'idle_b', 8.0, -8, -1, 49, 0.0, false, false, false)
							Wait(5000)
					...
					elseif action == 'handcuff' then
						...
						TriggerServerEvent('esx_gouvernmentjob:handcuff', GetPlayerServerId(closestPlayer), playerheading, playerCoords, playerlocation)
					elseif action == 'unhandcuff' then
						playerheading = GetEntityHeading(GetPlayerPed(-1))
						playerlocation = GetEntityForwardVector(PlayerPedId())
						playerCoords = GetEntityCoords(GetPlayerPed(-1))
						TriggerServerEvent('esx_gouvernmentjob:unhandcuff', GetPlayerServerId(closestPlayer), playerheading, playerCoords, playerlocation)
					elseif action == 'drag' then
						TriggerServerEvent('esx_gouvernmentjob:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_gouvernmentjob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_gouvernmentjob:OutVehicle', GetPlayerServerId(closestPlayer))
		...
		elseif data.current.value == 'vehicle_interaction' then
			...
			'default', GetCurrentResourceName(), 'vehicle_interaction',
			{
				...
				elseif DoesEntityExist(vehicle) then
					...
					elseif action == 'impound_gouvernment' then
						...
						CurrentTask.Task = ESX.SetTimeout(10000, function()
							ClearPedTasks(playerPed)
							ImpoundVehicleGouvernment(vehicle)
							Citizen.Wait(100) -- sleep the entire script to let stuff sink back to reality
						end)
		...
		elseif data.current.value == 'squad' then
			TriggerEvent("meeta_gouvernment:squadMenu")
		end
...
function OpenIdentityCardMenu(player)
	...
	ESX.TriggerServerCallback('esx_gouvernmentjob:getOtherPlayerData', function(data)
...
function OpenFineCategoryMenu(player, category)
	...
	ESX.TriggerServerCallback('esx_gouvernmentjob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category',
		{
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_gouvernment', _U('fine_total', label), amount)
			else
...
function SearchHistoryVehicleByName()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil then
			...
			ESX.TriggerServerCallback('esx_gouvernmentjob:getVehicleByName', function(result, found)
				if found then
...
function SearchHistoryHouseByName()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil then
			...
			ESX.TriggerServerCallback('esx_gouvernmentjob:getHouseByName', function(result, found)
				if found then
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_gouvernmentjob:getVehicleFromPlate', function(owner, found)
				if found then
					TriggerEvent("pNotify:SendNotification", {
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_gouvernmentjob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license',
		{
			...
			TriggerServerEvent('esx_gouvernmentjob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	...
	ESX.TriggerServerCallback('esx_gouvernmentjob:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	...
	ESX.TriggerServerCallback('esx_gouvernmentjob:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon',
		{
			...
			ESX.TriggerServerCallback('esx_gouvernmentjob:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
			end, data.current.value)
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon',
	{
		...
		ESX.TriggerServerCallback('esx_gouvernmentjob:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_gouvernmentjob:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_gouvernmentjob:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_gouvernmentjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_gouvernmentjob:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_gouvernmentjob:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_gouvernmentjob:forceBlip')
end)
...
AddEventHandler('esx_gouvernmentjob:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_gouvernmentjob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_gouvernmentjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if PlayerData.job ~= nil and PlayerData.job.name == 'gouvernment' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_gouvernmentjob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_gouvernmentjob:getarrested')
AddEventHandler('esx_gouvernmentjob:getarrested', function(playerheading, playercoords, playerlocation)
	...
	ESX.Streaming.RequestAnimDict("mp_arrest_paired", function()
		...
		Citizen.Wait(3060)
		TriggerEvent("esx_gouvernmentjob:handcuff")
	end)
	
end)

RegisterNetEvent('esx_gouvernmentjob:douncuffing')
AddEventHandler('esx_gouvernmentjob:douncuffing', function()
	Citizen.Wait(250)
...
RegisterNetEvent('esx_gouvernmentjob:getuncuffed')
AddEventHandler('esx_gouvernmentjob:getuncuffed', function(playerheading, playercoords, playerlocation)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	SetEntityCoords(GetPlayerPed(-1), x, y, z)
	SetEntityHeading(GetPlayerPed(-1), playerheading)
	Citizen.Wait(250)
...
RegisterNetEvent('esx_gouvernmentjob:doarrested')
AddEventHandler('esx_gouvernmentjob:doarrested', function()
	Citizen.Wait(250)
	ESX.Streaming.RequestAnimDict("mp_arrest_paired", function()
...
RegisterNetEvent('esx_gouvernmentjob:handcuff')
AddEventHandler('esx_gouvernmentjob:handcuff', function()
...
RegisterNetEvent('esx_gouvernmentjob:unrestrain')
AddEventHandler('esx_gouvernmentjob:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
...
RegisterNetEvent('esx_gouvernmentjob:drag')
AddEventHandler('esx_gouvernmentjob:drag', function(copID)
...
RegisterNetEvent('esx_gouvernmentjob:putInVehicle')
AddEventHandler('esx_gouvernmentjob:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_gouvernmentjob:OutVehicle')
AddEventHandler('esx_gouvernmentjob:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.gouvernmentStations) do
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'gouvernment' then
	...
	ขายสคริปอะไรก็ขอให้หลุด ชีวิตไม่เจริญ
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_gouvernmentjob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_gouvernmentjob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_gouvernmentjob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity ~= nil then
				TriggerEvent('esx_gouvernmentjob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'gouvernment' then
	...
	ขายสคริปอะไรก็ขอให้หลุด ชีวิตไม่เจริญ
				...
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBossMenuRL', 'gouvernment', function(data, menu)
						menu.close()
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'gouvernment' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'gouvernment_actions') then
			if Config.MaxInService == -1 then
				OpenGouvernmentActionsMenu()
			elseif playerInService then
				OpenGouvernmentActionsMenu()
			else
				ESX.ShowNotification(_U('service_not'))
...
RegisterNetEvent('esx_gouvernmentjob:updateBlip')
AddEventHandler('esx_gouvernmentjob:updateBlip', function()
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'gouvernment' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'gouvernment' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_gouvernmentjob:unrestrain')
	
	if not hasAlreadyJoined then
		TriggerServerEvent('esx_gouvernmentjob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_gouvernmentjob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'gouvernment')

		if Config.MaxInService ~= -1 then
			TriggerServerEvent('esx_service:disableService', 'gouvernment')
		end
...
function ImpoundVehicleGouvernment(vehicle)
	TriggerServerEvent('esx_gouvernmentjob:updateVehicleFromGouvernment', GetVehicleNumberPlateText(vehicle))
	ESX.Game.DeleteVehicle(vehicle) 
	TriggerEvent("pNotify:SendNotification", {
		text = '<strong class="green-text">พาวน์รถเรียบร้อยแล้ว</strong>',
		type = "success",
